...
my %opts;
my $USAGE = "USAGE: check.pl -d yyyymm [-a] file";

getopts('d:a', \%opts) or die $USAGE;
$armorial_mode = 1 if exists $opts{a};
if (exists $opts{d})
{
	die $USAGE unless $opts{d} =~ /^\d{6}$/;
	$lastdate = $opts{d};
}
my %count = ();

my %known_daud = ();
my $unicode_filename = '/Users/herveus/unicode/coa.tab';
...
my %known_category = ();
my %known_feature = ();
my $cat_filename = '/Users/herveus/aux/mike.cat';
...
my ($nonsca, $historical_type, $has_blazon, $combined, $name_only, $d1, $record);
  ...
  if ($note =~ /^For (.+)$/) {
    ...
    if ($note =~ /^transferred to (.+)$/) {
      ...
      &report("unexpected reblazoning note ($note) with blazonless type")
        unless ($has_blazon);
    } elsif ($note =~ /^(belatedly |erroneously |should have been )?(add|chang|convert|correct|redesignat|releas|remov|return|transferr)ed/) {
      # ok
    } else {
...
sub check_description {
  ...
  foreach my $feature (@features) {
    ...
    if ($feature eq 'proper' && $category !~ /^BIRD/) {
