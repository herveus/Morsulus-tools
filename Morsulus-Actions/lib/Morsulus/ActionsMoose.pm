...
sub as_str
{
	...
	return sprintf '%s', 
		join $SEPARATOR, $self->raw_form_of,
...
sub quote_names
{
		...
		/xsm)
	{
		...
		$cooked_action_of($cooked_form);
	}	
}

sub extract_quoted_names {
	my $self = shift;
	$self->quoted_names_of(
		 [ $self->cooked_form_of =~ m{ " ([^"]+) " }gxsm ]);
}
...
sub bracket_names
{
	...
	for my $qname (@{$self->quoted_names_of()})
	{
